...
/**
 * RefreshScope 注解可以实现动态加载配置信息（需要依赖 actuator 包），在服务启动后，当配置信息发生变动时，直接 POST 访问 http://
 * 服务ip:port/actuator/refresh（curl -X POST "http://localhost:3355/actuator/refresh"）即可实现配置的动态刷新（必须使用 POST
 * 请求，且必须要执行这一操作，否则不会配置刷新）。
 *
 * 例如，当更改数据库URL配置时，如果数据源有打开的连接，如果希望这些连接能继续完成正在做的工作，然后在下次从池中借用连接时，它才
 * 会得到一个带有新 URL 的连接。
 ...
 * 点用于公开此功能（通过HTTP或JMX）。如果需要按名称刷新单个bean，还有一个refresh（String）方法。
 *
 ...
 *
 * 要公开/refresh端点，需要向应用程序添加以下配置，即手动动态刷新：
 * management:
 *   endpoints:
 *     web:
 *       exposure:
