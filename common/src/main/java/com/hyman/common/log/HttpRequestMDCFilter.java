...
public class HttpRequestMDCFilter implements Filter {
    ...
    private void mdc(HttpServletRequest hsr) {
        ...
        MDC.put(MDCConstants.FROM, hsr.getHeader(MDCConstants.FROM));
        MDC.put(MDCConstants.USER_ID, hsr.getHeader(MDCConstants.USER_ID));
//        MDC.put(MDCConstants.CLIENT_IP, hsr.getHeader(MDCConstants.CLIENT_IP));
        MDC.put(MDCConstants.CLIENT_IP, getIp(hsr));
        MDC.put(MDCConstants.APPLICATION_NAME, applicationName);
        ...
        if (mappedHeaders) {
            ...
            if (e != null) {
                ...
                MDC.put(MDCConstants.HEADER_JSON, JSONObject.toJSONString(heads));
            }
        }
        ...
        if (mappedCookies) {
            ...
            if (cookies != null && cookies.length > 0) {
                Map<String, Object> cooks = new HashMap<>();
                for (Cookie cookie : cookies) {
                    String name = cookie.getName();
                    String value = cookie.getValue();
                    MDC.put(MDCConstants.COOKIE_KEY_PREFIX + name, value);
                    cooks.put(name, value);
                }
                MDC.put(MDCConstants.COOKIE_JSON, JSONObject.toJSONString(cooks));
            }
        }
        ...
        if (mappedParameters) {
            ...
            if (e != null) {
                ...
                MDC.put(MDCConstants.PARAMETER_JSON, JSONObject.toJSONString(params));
            }
        }
    }
